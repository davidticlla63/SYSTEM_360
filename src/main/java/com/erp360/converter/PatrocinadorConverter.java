...
@FacesConverter("patrocinadorConverter")
public class PatrocinadorConverter implements Converter {
		...
		if (value != null && value.trim().length() > 0) {
			try {
				...
				if (protocolo[protocolo.length - 1]
						.equalsIgnoreCase("encargado_venta.xhtml")) {
					if (protocolo[protocolo.length - 2]
							.equalsIgnoreCase("venta")) {
						pacientes = EjecutivoController.listPatrocinador;
					} 
				}
